...
/** Validates Twilio Voice webhooks when TWILIO_AUTH_TOKEN is set. */
...
export function registerTwilioRoutes(app: Express, authMiddleware: any) {
  ...
  /**
   * Parent leg TwiML: agent answers, then Dial lead with answered-party URL → callee Connect+Stream (bidirectional).
   */
...
<Response>
  ...
  /** Callee answered: bidirectional Media Stream so we can inject AI audio (mu-law) to PSTN. */
