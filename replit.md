...
-   **Twilio**: Connected via Replit integration (connector). Provides click-to-call, SMS, and **automatic call recording with AI intelligence pipeline**. Service module: `server/twilio-service.ts`. Routes: `server/twilio-routes.ts`. Every outbound Twilio call is automatically recorded (dual-channel). When recording completes, Twilio webhook triggers the full AI pipeline: download recording → Whisper transcription → deterministic containment analysis → GPT containment analysis → authority detection → follow-up date extraction → PostgreSQL storage (`twilio_recordings` table) → Airtable Calls writeback → EventBus `CALL_ANALYSIS_COMPLETE` event (feeds Sales Learning System: script evolution, pattern engine, observation engine). API endpoints: `GET /api/twilio/status`, `POST /api/twilio/call`, `POST /api/twilio/sms`, `GET /api/twilio/calls`, `GET /api/twilio/messages`, `GET /api/twilio/call/:sid`, `POST /api/twilio/webhook/recording` (Twilio callback), `GET /api/twilio/recordings`, `GET /api/twilio/recordings/:id`. Buttons appear in Call Mode only when Twilio is connected. SMS consent page at `/sms-consent` for Twilio A2P verification.
