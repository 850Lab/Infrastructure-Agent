...
-   **LNG Projects Intelligence Hub**: Client-specific feature (TCDT only) at `/machine/lng-projects`. Provides full web search for LNG projects, decision makers, and intel (hiring, events, press releases, contracts, etc.) using Outscraper + OpenAI analysis. Results can be saved to PostgreSQL (`lng_projects`, `lng_contacts`, `lng_intel` tables) with notes, filtering, and management. Nav button appears only for Texas Cool Down Trailers client. Backend: `server/lng-projects.ts`. Frontend: `client/src/pages/lng-projects.tsx`.
