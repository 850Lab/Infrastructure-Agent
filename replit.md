...
-   **Targeting Control Panel** (`/machine/targeting`): Operator-controlled lead targeting with Core Filters (industry, territory, role), Signal Filters (warm/stale/fresh/DM/phone/email toggles), Strictness Controls (broad/balanced/strict match modes), Priority Objectives (5 options), Required Field toggles (phone/DM/email/signal), live Target Summary bar, expandable Results Preview with Lead Explanation Layer, Coverage Diagnostics (exclusion reasons), Saved Target Profiles, and Batch Actions (Send to Focus, Follow-up Queue, Save as Segment, Export CSV, Mark Ignored). **Lead Explanation Layer**: Each lead row shows match reason badges (filter-driven, green), data signal badges (gray), priority reason badges (blue), and a recommended next action CTA. Expanding reveals 3-panel detail view + transcript-verified quality banner when available. **Transcript Quality Override**: After call recordings are analyzed by GPT-4o (`analyzeLeadQuality`), the verified score is written to `company_flows.verified_quality_score/label`. Leads scored 3/10 or below are automatically downgraded from "warm" even if the raw outcome was `live_answer`/`interested`. Backend: `POST /api/targeting/query`, `GET/POST/DELETE /api/targeting/profiles`, `POST /api/targeting/send-to-focus`, `POST /api/targeting/add-to-followup`. Data sources: `outreach_pipeline` + `company_flows` (cross-referenced for quality). Saved profiles stored in `target_profiles` table.
